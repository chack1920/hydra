...
func Test_serverHook_AddStarting(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "1.1 添加空服务", h: nil, wantErr: true, ErrStr: "启动服务不能为空"},
		{name: "1.2 添加服务", h: h},
		{name: "1.3 添加已存在的服务", h: h, wantErr: true, ErrStr: "启动服务不能重复注册"},
	}
	s := &serverHook{}
...
func Test_serverHook_AddClosing(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "1.2 添加服务", h: h},
		{name: "1.3 添加已存在的服务", h: h, wantErr: true, ErrStr: "关闭服务不能重复注册"},
	}
	s := &serverHook{}
	for _, tt := range tests {
